...
class WavePropagation(OpticalLayer):
    '''
    ...
    def get_propagation_operator(self, ka, kx, ky, z,L):
        ...
        :param L: D for antialiasing threshold (Evaluation of the angular spectrum approach for simulations of near field pressures, Xiaozheng Zeng, Robert J. McGough, 2007)
        ...
        D = L
        ## Extra threshold for aliasing effects (Xiaozheng Zeng, Robert J.McGough 2007)
    ...
    def get_propagation_matrix(self, N_in, N_out, z, dp, wavelength):
        ...
        r = tf.math.sqrt((jxpos * dp - ixpos * dp) ** 2 + (jypos * dp - iypos * dp) ** 2 + z ** 2)

        #print(tf.complex(wavelength,0.0))
        #print(tf.complex((1 / (2 * np.pi * r) + 1),0.0))
        #print(tf.complex((z / r ** 2),0.0))
    ...
    def __init__(self, z, N, L, padding = None, f0=2.00e6, cM=1484, channels_last = True, use_FT=True, **kwargs):
        ...
        '''
        super(WavePropagation, self).__init__(**kwargs)
        ...
        if use_FT:
            ...
            # We use original L (not padded) to get tighter antialiasing. Proofed to be usefull to ignore high k components.
    ...
    def call(self, input, **kwargs):
        ...
        if self.pad:
            ...
            #print(img.shape)
        #img = tf.image.pad_to_bounding_box(img, self.padding_pixel //2, self.padding_pixel//2, self.not_scaled_N, self.not_scaled_N)
    ...
    def call_for_fields(self, input, num = 100):
        ...
        z_arr = tf.linspace( 0.0, self.z_prop[0,0,0], num)
        for ind, el in enumerate(z_arr):
            z_current = tf.reshape(el, (1,1,1))
            #print(z_current)
            fields_propagation_operator = self.get_propagation_operator(self.ka, self.kx, self.ky, z_current, self.L)
            fields_inverse_propagation_operator = self.get_propagation_operator(self.ka, self.kx, self.ky, -z_current, self.L)

            ## transpose the channels, so that the image dimensions are last for fft
            if self.channels_last == True:
                img_t = tf.transpose(img, (0, 3, 1, 2))

            if self.use_FT:

                # calculate the 2d fourier transform of the wave
                fourier_pressure = tf.signal.fftshift(tf.signal.fft2d(tf.signal.ifftshift(img_t)))
                # Multiply by kspace propagator
                propagated_kspace = tf.multiply(fourier_pressure, fields_propagation_operator)
                # Retransform in image space
                propagated_image = tf.signal.fftshift(tf.signal.ifft2d(tf.signal.ifftshift(propagated_kspace)))
            else:
                img = tf.reshape(img_t, (img_t.shape[0], img_t.shape[1], self.N_in * self.N_in, 1))
                propagated_image = tf.matmul(self.w, img, True)
                propagated_image = tf.reshape(propagated_image, (img.shape[0], img.shape[1], self.N_out, self.N_out))
            ...
            fields.append(propagated_image)
