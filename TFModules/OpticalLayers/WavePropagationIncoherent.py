...
class WavePropagationIncoherent(OpticalLayer):
    '''
    ...
    def __init__(self, z, N, L, padding = None, f0=2.00e6, cM=1484, channels_last=True, use_FT=True,**kwargs):
        ...
        '''
        super(WavePropagationIncoherent, self).__init__(**kwargs)
    ...
    def call(self, input):
        ...
        if self.pad:
            #print(img.shape)
            #print(self.padding_layer.image_size)
            #print(self.N)
            #print(self.N_p)
        ...
        if self.use_FT:
            ...
            propagated_kspace = tf.complex(tf.math.abs(propagated_kspace),0.0)
            ...
            propagated_image = tf.matmul(tf.complex(tf.math.abs(self.w),0.0), tf.complex(tf.math.abs(img),0.0), True)
