...
class WavePropagationForwardOperator(ForwardOperator):
    '''
    ...
    def __init__(self, z, N, L, scale = 2, padding = None, f0=2.00e6, cM=1484, channels_last=True, use_FT=True, activation_function = None,**kwargs):
        '''
        ...
        '''
        super(WavePropagationForwardOperator, self).__init__(**kwargs)
        ...
        if padding == None:
            #calculate the needed padding with the diffraction limit of the airy disk
            #max_z = np.max(z)
            #self.padding = max_z/2* np.tan(np.arcsin( 1.22* self.wavelength/(self.dp * 1000)))
            #print(self.padding)
            self.padding = L/2
        else:
            self.padding = padding
        #print(self.padding)
        self.padding_pixel = int(self.padding / self.dp)+1
        #print(self.padding_pixel)
        self.not_scaled_N = (self.N + self.padding_pixel)
        self.actual_N = self.not_scaled_N*scale

        #print(self.padding_pixel)

        self.f0 = f0
        self.cM = cM
        self.df_p = 1 / (L + self.padding)
    ...
    def compute_output_shape(self, input_shape):
        return [input_shape[0], self.N, self.N,len(self.z)]
    # @tf.function
    def call(self, input):
        '''
        ...
        img = repeat_image_tensor(tf.image.pad_to_bounding_box(img, self.padding_pixel // 2, self.padding_pixel // 2, self.not_scaled_N, self.not_scaled_N),self.scale)
        ...
        if self.use_FT:
            ...
            #print(self.propagation_operator.shape)
        ...
        #    propagated_image = self.activation_function(propagated_image)
    ...
    def inverse_call(self, input):
        ...
        img = repeat_image_tensor(
            tf.image.pad_to_bounding_box(img, self.padding_pixel // 2, self.padding_pixel // 2, self.not_scaled_N,
                                         self.not_scaled_N), self.scale)
        ...
        if self.use_FT:
            ...
            #print(fourier_pressure.shape)
            #print(self.propagation_operator.shape)
            #print(self.inverse_propagation_operator.shape)
