...
class TensorboardDDNNCategoricalCallback(tf.keras.callbacks.Callback):
    def __init__(self, test_data, PATH= '.', images_output_steps = 100, loss_output_steps = 100, print_propagation_images = True):
        super(TensorboardDDNNCategoricalCallback, self).__init__()        
        self.PATH = PATH
    ...
    def on_train_batch_end(self, batch, logs=None):
        if batch % self.images_output_steps == 0:
            ...
            for i in range(0,result.shape[0]):
                ...
                category_img = self.model.get_class_image(np.argmax(self.wanted_output[i,:]))

                wanted_img = ax[2].imshow(category_img)
                ax[2].set_title('Wanted Output mask')
