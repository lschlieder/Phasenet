...
class OpticalFunctionApproximatorPhase(DDNN):
    def __init__(self, encoding = 'IntensityPhase', num_layers = 3, propagation_distance = [0.08], layers_trainable = 'all', offset = False, intensity_output = True, encoding_pixels = 112, 
                 radius = 0.001, phase_uncertainty = None, fourier_components:int = -1, use_regularizer = False, lam = 0.001, output_distance = 0.006, output_function = 'sum', output_category_shape = 'hex', negative_output = False, mean_size = 112, trainable_pixel=112, plate_scale_factor=1,
            ...
            super(OpticalFunctionApproximatorPhase,self).__init__(trainable_pixel, plate_scale_factor, propagation_size, propagation_pixel, padding, frequency, wavespeed, **kwargs)
            ...
            self.phase_uncertainty = phase_uncertainty
            ...
            if layers_trainable == 'all':
                layers_trainable = range(0,num_layers)
            #self.mean_size = mean_size
            #input_shape = [None, self.propagation_pixel, self.propagation_pixel, 1]
            #input handling
            input_shape = [None, 1]

            assert( encoding in ['Intensity', 'Phase', 'IntensityPhase', 'RealSetup','Fourier'])
            ...
            if encoding == 'IntensityPhase':
                ...
                input_shape = self.append_element(ENC.RegressionEncodingIntensity(encoding_pixels, sigma), input_shape)
            elif encoding == 'Phase':
                input_shape = self.append_element(ENC.RegressionEncodingPhase(encoding_pixels, sigma), input_shape)
            elif encoding == 'RealSetup':
                input_shape = self.append_element(ENC.RegressionEncodingPhaseSplitGaussian(propagation_size, encoding_pixels, sigma), input_shape)
            elif encoding == 'Fourier':
                assert(fourier_components > 0)
                input_shape = self.append_element(ENC.RegressionEncodingFourier(propagation_size, fourier_components, encoding_pixels, sigma), input_shape)
            ...
            self.plates = []
            def get_plate(amp, phase):
                if self.phase_uncertainty == None:
                    return self.get_phaseplate(amp,phase)
                else:
                    return self.get_uncertainty_phaseplate (False, True, None, self.phase_uncertainty)
            ...
            if self.offset: 
                output_shape = self.append_element(OL.CategoryOutputLayer(1, (input_shape[1], input_shape[2]), [propagation_size, propagation_size], radius, output_distance, output_shape = output_category_shape, output_function = output_function, negative_output = negative_output),input_shape)
                output_shape = self.append_element(NL.OffsetMultiplicationLayer(use_regularizer=use_regularizer, lam = lam), input_shape)
            else: 
                output_shape = self.append_element(OL.CategoryOutputLayer(1, (input_shape[1], input_shape[2]), [propagation_size, propagation_size], radius, output_distance, output_shape = output_category_shape, output_function = output_function, negative_output = negative_output),input_shape)

            #output_shape = self.append_element(OL.CategoryOutputLayer(categories, (input_shape[1], input_shape[2]), [propagation_size + padding, propagation_size + padding], radius, distance, output_shape = output_shape, output_function = output_function, negative_output = negative_output),input_shape)
    ...
    def call(self,Input):
        ...
        #u = u / self.scale_var
        return u
    def get_nonscaled_output(self, Input):
        u = Input
        if self.offset:
