...
class CircularOutput(DDNN):
    def __init__(self, axis = 'xy', trainable_polarization = 'xy',phase_input = False, trainable_pixel = 112, plate_scale_factor = 2, propagation_size = 0.001792, propagation_pixel = 224, padding = 0.00175, frequency = 3.843e14, wavespeed = 3e8, **kwargs):
        super(CircularOutput,self).__init__(trainable_pixel, plate_scale_factor, propagation_size, propagation_pixel, padding, frequency, wavespeed, **kwargs)
        ...
        input_shape = self.append_element(OL.PhaseRetarder(rot_angle = np.pi/4), input_shape)
        input_shape = self.append_element(self.get_jones_phaseplate(False,True, trainable_polarization), input_shape)
        input_shape = self.append_element(self.get_wave_propagation([0.08], input_shape[1]),input_shape)
        input_shape = self.append_element(self.get_jones_phaseplate(False,True, trainable_polarization), input_shape)
        input_shape = self.append_element(self.get_wave_propagation([0.13], input_shape[1]),input_shape)
        #output
        input_shape = self.append_element(OL.CircularPolarizationFilter('left'), input_shape)
