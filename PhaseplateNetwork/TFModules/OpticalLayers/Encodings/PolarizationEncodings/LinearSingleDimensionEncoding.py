...
class LinearSingleDimensionEncoding(OpticalLayer):
    def __init__(self, axis = 'x', **kwargs):
        super(LinearSingleDimensionEncoding,self).__init__(**kwargs)
    ...
    def call(self,input):
        ...
        if self.axis =='x':
            output = tf.complex(abs,0.0)*tf.math.exp(1j*tf.complex(phase,0.0)) * self.x + tf.complex(tf.ones_like(abs), 0.0) * tf.math.exp(1j * tf.complex(tf.zeros_like(phase), 0.0)) * self.y
        elif self.axis == 'y':
            output = tf.complex(tf.ones_like(abs), 0.0) * tf.math.exp(1j * tf.complex(tf.zeros_like(phase), 0.0)) * self.x + tf.complex(
