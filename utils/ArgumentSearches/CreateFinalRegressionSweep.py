...
datasets = ['regression_x2exp', 'regression_small_step', 'regression_big_gaussian', 'regression_sin3x', 'regression_sin2x', 'regression_neg_sin']
...
output = '/is/cluster/work/lschlieder/DDNN/UniversalFunctionApproximator/Regression/'

output_strs = [[f'''network_class OpticalFunctionApproximatorPhase -o {output}{i_dataset}/encoding:{i_encoding}/negative_output_weights:{i_neg_output =='--negative_output True '}
/radius:{i_radius:.6f}/Layers:{i_layers}_offset:True 
-d {i_dataset} --num_layers {i_layers} {i_neg_output}--radius {i_radius} --output_distance 0.0008 --offset True --output_function mean 
--callbacks TB TB-Regression ModelCheckpoint BackupAndRestore --encoding {i_encoding}'''.replace('\n', '')
...
argument_files = [f"/home/lennart/Desktop/CLUSTER_HOME_NET/Arguments/Optical/Regression/RegressionPaperRuns_fitting_radius_neg_output_{i_dataset}.txt" for i_dataset in datasets]
